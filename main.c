...
{
        /**
         * incase there is environment variable
         * substitute it's value befgore command execution
         */
        if (argv[1] && argv[1][0] == '$'){
        //argv[1] = putenv(argv[1]);
        ...
        if(child_pid == 0){
            ...
            if(in_background == 1){
                waitpid(child_pid,&status,WNOHANG);    // return immediately (don't wait for the child process to terminate)
            }
